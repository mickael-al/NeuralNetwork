...
void main() 
{
	...
	vec3 camToVertex = ubc.camPos - (ubo[index_ubo].model * vec4(0, 0, 0, 1)).xyz;
	...
	vec3 newInPos = rotationMatrixY * inPosition;

	WorldPos = vec3(ubo[index_ubo].model * vec4(newInPos, 1.0));
	Color = inColor;
	vec3 T = normalize(vec3(ubo[index_ubo].model * vec4(rotationMatrixY*inTangent, 0.0)));
	vec3 N = normalize(vec3(ubo[index_ubo].model * vec4(rotationMatrixY*inNormal, 0.0)));	
	T = normalize(T - dot(T, N) * N);
	vec3 B = cross(N, T);
	TBN = mat3(T, B, N);
	imaterial = index_material;
	ViewPos = (ubc.view * ubo[index_ubo].model * vec4(newInPos, 1.0)).xyz;
    gl_Position = ubc.proj * ubc.view * ubo[index_ubo].model * vec4(newInPos, 1.0);
}