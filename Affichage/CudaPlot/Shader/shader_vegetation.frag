...
{   
    ...
    vec3 ambient = vec3(0.03) * color * ubm[imaterial].ao * texture(texSampler[ubm[imaterial].aoMap], fragTexCoord).rgb;
    ...
    float n = remap(cnoise(WorldPos.xz*0.25+vec2(sin(ubd.u_time*0.3),cos(ubd.u_time*0.3)),0.4), 0.0, 1.0, 0.65, 1.0);    
    color = mix(color, vec3(n), fogFactor);
