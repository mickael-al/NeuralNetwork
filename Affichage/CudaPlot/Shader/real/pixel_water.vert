...
{	
	...
	vec3 newPosition = WorldPos + vec3(0.0, amplitude * sin(ubd.u_time * frequency + (WorldPos.x + WorldPos.z) * sizePos), 0.0);
	WorldPos = newPosition; 
    imaterial = index_material;
	ViewPos = (ubc.view * ubo[index_ubo].model * vec4(inPosition, 1.0)).xyz;
    gl_Position = ubc.proj * ubc.view * vec4(WorldPos,1);
}