...
namespace Ge
{
	...
	bool Hud::initialize(VulkanMisc* vM)
	{
		...
		poolInfo.maxSets = 100;//valeur aleatoire attention
	...
	FontData * Hud::createFont(const char* filename, float size_pixels, const ImFontConfig* font_cfg, const ImWchar* glyph_ranges)
	{
		...
		Textures* texture = new Textures(pixels, width, height, -1, false, vulkanM);
			
		VkDescriptorSet textdesc = ImGui_ImplVulkan_AddTexture(texture->getVkSampler(), texture->getVkImageView(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
		io.Fonts->SetTexID((ImTextureID)textdesc);
		...
		m_fontdata.push_back(fd);
		return fd;
	}

	void Hud::destroyFont(FontData * font)
	{				
		ImGuiIO& io = ImGui::GetIO();
		m_fontdata.erase(std::remove(m_fontdata.begin(), m_fontdata.end(), font), m_fontdata.end());
		delete font->texture;
		ImGui_ImplVulkan_RemoveTexture(font->desc);		
		if (io.Fonts->GetTexID() == font->font)
		{
	...
	{
		ImGuiIO& io = ImGui::GetIO();
		for (int i = 0; i < m_fontdata.size();i++)
		{
			delete m_fontdata[i]->texture;
			ImGui_ImplVulkan_RemoveTexture(m_fontdata[i]->desc);
			delete m_fontdata[i];
		}
		m_fontdata.clear();
		io.Fonts->Clear();
