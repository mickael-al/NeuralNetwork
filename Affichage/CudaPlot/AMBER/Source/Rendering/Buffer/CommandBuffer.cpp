...
namespace Ge
{
	...
	bool CommandBuffer::initialize(ShadowManager * shadowManager, PostProcessing postProcessing,VulkanMisc * vM, ptrClass * ptrC)
	{
		...
		for (size_t i = 0; i < m_commandBuffers.size(); i++)
		{
			...
			std::array<VkDescriptorSet, 4> shadowDesciptor { (shadowManager->getDescriptor()[1])->getDescriptorSets(), all_descriptor[1]->getDescriptorSets(), all_descriptor[2]->getDescriptorSets(), all_descriptor[3]->getDescriptorSets() };
			...
			for (int j = 0; j < shadows.size(); j++)
			{
				...
				for (int k = 0; k < currentShadowFrames.size(); k++)
				{
					...
					PushConstantShadow pcs{ globalShadowFrameCount };
