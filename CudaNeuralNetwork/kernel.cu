...
__device__ double TanhDerive(double x)
{
	return 1.0 - (x*x);
}

__global__ void propagateNeuralNetworkCompact(const NeuralSwapData* nld, const NeuralNetworkDataCompact* nndc,const int * self_d, double*** self_w, double** self_x)
{
...
__global__ void backPropagateNeuralNetworkCompact(const NeuralSwapData* nld,const int* self_d, double*** self_w, double** self_x, double** self_delta)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= nld->size || i == 0)
	{
...
__global__ void backPropagateNeuralNetworkCompactEnd(const NeuralSwapData* nld, const NeuralNetworkDataCompact* nndc,const int* self_d, double*** self_w, double** self_x, double** self_delta)
{
	int i = blockIdx.x * blockDim.x + threadIdx.x;
	if (i >= nld->size)
	{
